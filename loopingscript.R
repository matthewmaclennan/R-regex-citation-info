...
totalresults<-xpathApply(xmlTreeParse(getURL("api.elsevier.com/content/search/scidir?apiKey=____&query=TITLE-ABS-KEY%28capillary+electr*+derivati*%29&httpAccept=%20application%2Fxml&start=0&count=100")
,useInternalNode=T),"//opensearch:totalResults/text() div 1")
refreshes<-floor(totalresults/100)
results<-c()
  for(i in 0:refreshes*100){
    resultpage<-getURL(paste0("api.elsevier.com/content/search/scidir?apiKey=_____&query=TITLE-ABS-KEY%28capillary+electr*+derivati*%29&httpAccept=%20application%2Fxml&start=",i,"&count=100"))
